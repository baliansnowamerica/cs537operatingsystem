...
clone(void(*fcn) (void *, void *), void *arg1, void *arg2, void *stack) {
  ...
  np->parent = proc;
  *np->tf = *proc->tf;
  // cprintf("clone proc->tf->eax: %d\n", proc->tf->eax);
  np->tf->eax = 0;

  // set up a kernel stack for the new thread
  // stack base pointer
  np->tstack = (uint) stack;
  cprintf("clone np->tf->ebp: %d\n", np->tf->ebp);
  ...
  uint sp = np->tstack + PGSIZE - sizeof(uint)*3;
